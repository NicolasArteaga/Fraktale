...
int main(int argc, char** argv ){
    ...
    int width = (r_end-r_start)*res;                //Re range of [r_start,r_end]
    int height = ((im_end-im_start)*res);         //Im range of [im_start,im_end]
    ...
    //Creat buffer for pixel array:
    unsigned char* buf = malloc(pixelNum);
    ...
    multicorn_c(r_start,r_end,im_start,im_end,res,buf);

    //Benchmarking
    clock_gettime(CLOCK_MONOTONIC, &t2);
    double time = ((t2.tv_sec - t1.tv_sec)+((t2.tv_nsec*1e-9)-(t1.tv_nsec * 1e-9)));

    FILE *fout = fopen("result_c.bmp", "wb");
    fwrite(header, 1, 54, fout);
    fwrite((char*)buf, 1, pixelNum, fout);
    fclose(fout);
    free(buf);
    printf("result_c.bmp created\n");
